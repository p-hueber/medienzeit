...
//!
//! Generic over the identity types so this can live here rather than in the firmware,
...
///
/// `T` is the primary identity (an NFC tag) and `C` a secondary one (a card, during a
/// changeover). A device may have either, both, or neither; with neither it falls back
/// to whatever the caller passes, which is how a button stands in before tags exist.
pub struct Docking<T, C, const N: usize> {
    known: [Option<T>; N],
    known_cards: [Option<C>; N],
    last: [bool; N],
...
impl<T: Copy + PartialEq, C: Copy + PartialEq, const N: usize> Docking<T, C, N> {
    pub fn new(known: [Option<T>; N], known_cards: [Option<C>; N]) -> Self {
        Self::with_threshold(known, known_cards, DEFAULT_FAILURES_BEFORE_ALERT)
    }
    ...
    pub fn update(
        ...
        for (i, slot) in docked.iter_mut().enumerate() {
            // A tag wins if one is configured; otherwise a card; otherwise the fallback.
            // Both can be configured during a changeover, and either being present
            // counts as put back, so swapping identities needs no flag day.
            let by_tag = self.known[i].map(|id| seen.contains(&id));
            let by_card = self.known_cards[i].map(|id| card == Some(id));
            *slot = match (by_tag, by_card) {
                (Some(a), Some(b)) => a || b,
                (Some(a), None) => a,
                (None, Some(b)) => b,
...
mod tests {
    ...
    type D = Docking<u8, u8, 2>;

    fn with_tags() -> D {
        Docking::new([Some(1), Some(2)], [None, None])
    }

    #[test]
    fn a_tag_in_the_field_means_put_back() {
        let mut d = with_tags();
        assert_eq!(d.update(Some(&[1, 2]), None, [false; 2]).docked, [true, true]);
        assert_eq!(d.update(Some(&[1]), None, [false; 2]).docked, [true, false]);
        assert_eq!(d.update(Some(&[]), None, [false; 2]).docked, [false, false]);
    }
    ...
    fn it_starts_docked() {
        ...
        assert_eq!(d.update(None, None, [false; 2]).docked, [true, true]);
    }
    ...
    fn a_reader_failure_holds_the_last_state() {
        let mut d = with_tags();
        d.update(Some(&[1, 2]), None, [false; 2]);
        for _ in 0..20 {
            assert_eq!(
                d.update(None, None, [false; 2]).docked,
                [true, true],
    ...
    fn a_failure_holds_a_taken_device_taken_too() {
        let mut d = with_tags();
        d.update(Some(&[]), None, [false; 2]);
        assert_eq!(d.update(None, None, [false; 2]).docked, [false, false]);
    }
    ...
    fn an_empty_reading_is_not_a_failure() {
        let mut d = with_tags();
        d.update(Some(&[1, 2]), None, [false; 2]);
        let r = d.update(Some(&[]), None, [false; 2]);
    ...
    fn the_fault_alert_fires_once_not_every_tick() {
        let mut d = Docking::<u8, u8, 2>::with_threshold([Some(1), Some(2)], [None, None], 3);
        assert!(!d.update(None, None, [false; 2]).reader_fault);
        assert!(!d.update(None, None, [false; 2]).reader_fault);
        assert!(d.update(None, None, [false; 2]).reader_fault, "fires at the threshold");
        for _ in 0..10 {
            assert!(
                !d.update(None, None, [false; 2]).reader_fault,
    ...
    fn recovering_rearms_the_fault_alert() {
        let mut d = Docking::<u8, u8, 2>::with_threshold([Some(1), Some(2)], [None, None], 2);
        d.update(None, None, [false; 2]);
        assert!(d.update(None, None, [false; 2]).reader_fault);
        d.update(Some(&[1]), None, [false; 2]);
        d.update(None, None, [false; 2]);
        assert!(d.update(None, None, [false; 2]).reader_fault, "a second outage alerts again");
    }

    #[test]
    fn an_unknown_tag_is_reported_once_per_identity() {
        let mut d = with_tags();
        assert_eq!(d.update(Some(&[9]), None, [false; 2]).unknown, Some(9));
        for _ in 0..5 {
            assert_eq!(
                d.update(Some(&[9]), None, [false; 2]).unknown,
                None,
                "the same stray tag must not alert every second"
            );
        }
        assert_eq!(d.update(Some(&[8]), None, [false; 2]).unknown, Some(8));
    }

    #[test]
    fn a_known_tag_is_never_unknown() {
        let mut d = with_tags();
        assert_eq!(d.update(Some(&[1, 2]), None, [false; 2]).unknown, None);
    }
    ...
    fn a_device_without_an_identity_uses_the_fallback() {
        let mut d: D = Docking::new([Some(1), None], [None, None]);
        assert_eq!(d.update(Some(&[1]), None, [false, true]).docked, [true, true]);
        assert_eq!(d.update(Some(&[1]), None, [false, false]).docked, [true, false]);
    }

    #[test]
    fn a_card_identifies_a_device_too() {
        let mut d: D = Docking::new([None, None], [Some(7), None]);
        assert!(d.update(Some(&[]), Some(7), [false; 2]).docked[0]);
        assert!(!d.update(Some(&[]), Some(8), [false; 2]).docked[0]);
        assert!(!d.update(Some(&[]), None, [false; 2]).docked[0]);
    }

    /// During a changeover a device may carry both. Either one counts, so the switch
    /// needs no coordinated moment.
    #[test]
    fn either_identity_counts_when_both_are_configured() {
        let mut d: D = Docking::new([Some(1), None], [Some(7), None]);
        assert!(d.update(Some(&[1]), None, [false; 2]).docked[0]);
        assert!(d.update(Some(&[]), Some(7), [false; 2]).docked[0]);
        assert!(!d.update(Some(&[]), None, [false; 2]).docked[0]);
    }
    ...
    fn unconfigured_only_when_nothing_is_set() {
        let none: D = Docking::new([None, None], [None, None]);
        assert!(none.unconfigured());
        let tag: D = Docking::new([Some(1), None], [None, None]);
        assert!(!tag.unconfigured());
        let card: D = Docking::new([None, None], [None, Some(7)]);
        assert!(!card.unconfigured());
    }
}
